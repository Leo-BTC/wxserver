...
jQuery(function () {
    ...
    $(document).ready(function () {
        $table.bootstrapTable({
            url: '/sysmsg/msg/get_received_list',  // table 请求路径
            toolbar: ".toolbar",
            showRefresh: true,
            search: true,
            showToggle: true,
    ...
    function formatState(value, row, index) {
        if (value == 0){
            return ['<button class="btn btn-round btn-xs btn-info btn-fill"  disabled="true">未读</button>'];
        }
        return ['<button class="btn btn-round btn-xs btn-success btn-fill"  disabled="true">已读</button>'];
    }
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        // //删除选择的选项
                        $.ajax({
                            url: "/sysmsg/msg_receiver_delete/" + result,
                            type: 'POST',
                            data: {},
...
function btn_detail(id) {
    window.location.href = "/sysmsg/get_detail/" + id+"/0";
}
